...
export default function Accounts({ mode, selectedId }:{mode:string,selectedId:number}) {
    ...
    const handleDeleteContact = async (contactId: number, index: number) => {
        if (selectedId) {
            try {
                await ApiService.deleteAccountContact(selectedId, contactId);
                // Optionally, remove contact from UI state after deletion
        ...
        <>
            ...
            >
                    ...
                    {  formik.values.contacts.map((contact, index) =>
                        ...
                        <div  key={index} className='  flex flex-col tertiary-color px-5 py-7  gap-6 my-7 rounded w-full '>
                            <AddImageBtn/>
                        <div
                             className='flex justify-around  my-5 gap-6'>
                        ...
                        <input type='submit'
                                ...
                                onClick={() => handleDeleteContact(contacts[index].id, index)}
                        />
            </div>
        </div>
                    )
